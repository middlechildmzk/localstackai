...
import { ExternalLink, GitFork, Bookmark, Flag, Star } from "lucide-react";
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex items-start gap-5 mb-8">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              ...
              <span className={`badge ${freshnessColor(tool.freshness).replace("text-", "badge-")}`}>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Link
                href={`/stacks/new?tool=${tool.slug}`}
                className="inline-flex items-center gap-2 px-4 py-2 border border-white/10 hover:border-white/20 text-zinc-300 text-sm rounded-lg transition-all"
              >
                <GitFork size={13} /> Add to Stack
              </Link>
              <Link
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {(tool.best_for?.length || tool.not_ideal_for?.length) && (
              <section className="glass p-5 grid grid-cols-2 gap-4">
            ...
            {stacks.length > 0 && (
              <section className="glass p-5">
                <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-widest mb-3">
                  Used in {stacks.length} stack{stacks.length !== 1 ? "s" : ""}
                </h2>
                <div className="flex flex-wrap gap-2">
                  {stacks.slice(0, 8).map((st: any) => (
                    <Link
                      key={st.stack?.id}
                      href={`/stacks/${st.stack?.slug}`}
                      className="px-3 py-1.5 text-xs bg-white/5 border border-white/5 rounded-lg text-zinc-400 hover:text-white hover:border-white/10 transition-all"
                    >
                      {st.stack?.title}
                    </Link>
                  ))}
            ...
            {alternatives.length > 0 && (
              <section className="glass p-5">
                ...
                <div className="space-y-2">
                  {alternatives.slice(0, 4).map((rel: any) => {
                    const alt = rel.target_tool;
                    if (!alt) return null;
                    return (
                      <Link
                        key={alt.id}
                      ...
                      >
                        <div className="w-7 h-7 rounded bg-zinc-800 flex items-center justify-center text-xs font-bold text-zinc-500 shrink-0">
                          {alt.name[0]}
                        </div>
                        <div className="flex-1 min-w-0">
                          <span className="text-sm text-white">{alt.name}</span>
                          {alt.tagline && (
                            <span className="text-xs text-zinc-600 ml-2 truncate">
                              {alt.tagline}
                            </span>
                          )}
                        </div>
                        <span className="text-xs text-zinc-600 shrink-0">
                          {formatPrice(alt.starting_price, alt.pricing_model)}
                        </span>
                      </Link>
                    );
                  })}
          ...
          <div className="space-y-4">
            ...
            <div className="glass p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                {tool.starting_price && (
                  <div className="flex justify-between">
                    <span className="text-zinc-500">Starts at</span>
                    <span className="text-white">${tool.starting_price}/mo</span>
                  </div>
                )}
                {tool.pricing_notes && (
                  <p className="text-xs text-zinc-600 mt-2 pt-2 border-t border-white/5">
                    {tool.pricing_notes}
                  </p>
                )}
              </div>
            ...
            <div className="glass p-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className={freshnessColor(tool.freshness)}>
                    {freshnessLabel(tool.freshness)}
                  </span>
                </div>
                {tool.last_verified_at && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-zinc-400">
                      {timeAgo(tool.last_verified_at)}
                    </span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-zinc-500">Stack count</span>
                  <span className="text-zinc-400">{tool.stack_count}</span>
                </div>
              </div>
            </div>
            ...
            <div className="glass p-4 space-y-2">
              {!tool.is_claimed && (
                ...
                >
                  <Star size={13} /> Are you the maker? Claim this profile
                </Link>
              )}
              <Link
                href={`/submit?correction=${tool.slug}`}
                className="flex items-center gap-2 text-sm text-zinc-400 hover:text-white transition-colors"
              >
                <Flag size={13} /> Suggest a correction
              </Link>
            </div>
          </div>
        </div>
