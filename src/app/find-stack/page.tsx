  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f]">
      <section className="relative overflow-hidden border-b border-white/5 px-4 py-16">
        ...
        <div className="relative mx-auto max-w-5xl text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-brand-500/30 bg-brand-500/10 px-3 py-1.5 text-xs font-medium text-brand-300">
            <Sparkles size={13} /> Public beta stack finder
          </div>
          <h1 className="mb-5 text-4xl font-bold tracking-tight text-white sm:text-6xl" style={{ fontFamily: "var(--font-display)" }}>
            Find the right AI stack for what you want to build.
          </h1>
          <p className="mx-auto max-w-3xl text-lg leading-8 text-zinc-400">
            Describe your workflow and StackBuilder will map it to relevant workflows, tools, and starter stack options.
          </p>
          <form action="/find-stack" className="mx-auto mt-8 flex max-w-2xl flex-col gap-3 rounded-2xl border border-white/10 bg-[#111118] p-3 sm:flex-row">
            <div className="relative flex-1">
              ...
              <input name="q" defaultValue={query} placeholder="Try: AI stack for TikTok videos, music artists, recruiters..." className="h-12 w-full rounded-xl border border-white/10 bg-black/30 pl-11 pr-4 text-sm text-white outline-none placeholder:text-zinc-600 focus:border-brand-500/60" />
            </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 space-y-12">
        ...
        <Results title="Workflow matches" href="/workflows">
          {(workflowResults.length ? workflowResults : allWorkflows).slice(0, 6).map((workflow) => <WorkflowCard key={workflow.id} workflow={workflow} />)}
        </Results>
        <Results title="Tool matches" href={query ? `/tools?q=${encodeURIComponent(query)}` : "/tools"}>
          {(toolResults.length ? toolResults : allTools).slice(0, 9).map((tool, i) => <ToolCard key={tool.id} tool={tool} rank={i + 1} />)}
        </Results>
      </section>
    </div>
  );
...
function ResultHeader({ query }: { query: string }) {
  return <div className="rounded-2xl border border-brand-500/20 bg-brand-500/10 p-5"><p className="text-sm text-brand-200">Showing matches for:</p><h2 className="mt-1 text-2xl font-bold text-white">“{query}”</h2></div>;
}
...
function RecommendedWorkflow({ workflow, stackTools, query }: { workflow: Workflow; stackTools: Tool[]; query: string }) {
  return (
    <section className="rounded-3xl border border-white/10 bg-[#111118] p-6">
      <p className="text-sm font-medium text-brand-400">Recommended workflow</p>
      <h2 className="mt-1 text-2xl font-bold text-white" style={{ fontFamily: "var(--font-display)" }}>{workflow.title}</h2>
      {workflow.description && <p className="mt-3 max-w-3xl text-sm leading-6 text-zinc-400">{workflow.description}</p>}
      {stackTools.length > 0 && <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">{stackTools.slice(0, 5).map((tool, index) => <Link key={tool.id} href={`/tools/${tool.slug}`} className="rounded-2xl border border-white/10 bg-black/20 p-4 hover:border-brand-500/40"><div className="mb-2 text-xs font-mono text-zinc-600">Step {index + 1}</div><div className="font-semibold text-white">{tool.name}</div><p className="mt-2 line-clamp-3 text-xs leading-5 text-zinc-500">{tool.tagline}</p></Link>)}</div>}
      <div className="mt-6 flex flex-col gap-3 sm:flex-row"><Link href="/stacks/new" className="inline-flex items-center justify-center rounded-xl bg-brand-600 px-5 py-3 text-sm font-semibold text-white hover:bg-brand-500">Build this stack</Link><Link href={query ? `/tools?q=${encodeURIComponent(query)}` : "/tools"} className="inline-flex items-center justify-center rounded-xl border border-white/10 px-5 py-3 text-sm text-zinc-300 hover:border-white/20">Browse matching tools</Link></div>
    </section>
  );
}

function Results({ title, href, children }: { title: string; href: string; children: React.ReactNode }) {
  return <section><div className="mb-6 flex items-end justify-between gap-4"><h2 className="text-2xl font-bold text-white" style={{ fontFamily: "var(--font-display)" }}>{title}</h2><Link href={href} className="text-sm text-brand-400 hover:text-brand-300">View all →</Link></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">{children}</div></section>;
}

const INTENT_TERMS: Record<string, string[]> = {
  tiktok: ["short-form", "shorts", "reels", "video", "caption", "social", "creator"],
  youtube: ["video", "faceless", "script", "voice", "editing", "thumbnail"],
  faceless: ["youtube", "video", "script", "voice", "narration", "editing"],
  music: ["audio", "song", "suno", "udio", "artist", "video", "social"],
  recruiter: ["sourcing", "research", "candidate", "outreach", "productivity", "automation"],
  podcast: ["audio", "clips", "short-form", "transcript", "video", "social"],
